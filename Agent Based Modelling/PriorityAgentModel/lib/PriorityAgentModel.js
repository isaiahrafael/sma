	...
	switch(state){
		...
		case WAITING:
			switch (type){
				...
				break;
				case "B":
					if (nextTreatedPatientID_A > nextPatientID_A && patient.id == nextTreatedPatientID_B){
						patient.target.row = doctorRow-1;
						patient.target.col = doctorCol+1;
						patient.state = STAGING;
					}
					if (nextTreatedPatientID_A > nextPatientID_A && patient.id == nextTreatedPatientID_B + 1){
						patient.target.row = doctorRow-1;
						patient.target.col = doctorCol+2;
					}
				break;
		...
		case STAGING:
			...
			if (hasArrived) {
				if (doctor.state == IDLE) {
					// Ensure Type "A" patients get treated first
					var anyAInStaging = patients.some(p => p.state === STAGING && p.type === "A");
		
					if (patient.type === "A" || !anyAInStaging) {
						doctor.state = BUSY;
						...
						if (patient.type == "A") {
							nextTreatedPatientID_A++;
						} else {
							nextTreatedPatientID_B++;
						}
					}
				}
			}
